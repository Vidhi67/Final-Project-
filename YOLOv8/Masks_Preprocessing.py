...
image_file_dir = '../Preprocessing/preprocessed_img/'
#########################################
...
def copy_files(file_list, source_image_dir, source_label_dir, target_image_dir, target_label_dir):
    for file_name in file_list:
        # Copy image file
        image_filename = f'{os.path.splitext(file_name)[0]}.jpg'
        if os.path.exists(os.path.join(source_label_dir, image_filename)):
            shutil.copy(os.path.join(source_image_dir, image_filename), os.path.join(target_image_dir, image_filename))

        # Copy corresponding label file
        label_filename = f'{os.path.splitext(file_name)[0]}.txt'
