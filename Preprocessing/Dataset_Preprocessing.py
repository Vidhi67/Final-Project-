...
def main():
    # Load data
    df_temp = pd.read_csv('../Datasets/JustRAIGS_Train_labels.csv', sep=';')
    ...
    # Save to CSV
    train_rg.to_csv('../Datasets/10_features_no_mask_train.csv', index=False)  #Check the path of the files and code accordingly
    test_rg.to_csv('../Datasets/10_features_no_mask_test.csv', index=False)
    train_glaucoma.to_csv('../Datasets/glaucoma_no_mask_train.csv', index=False)
    test_glaucoma.to_csv('../Datasets/glaucoma_no_mask_test.csv', index=False)
